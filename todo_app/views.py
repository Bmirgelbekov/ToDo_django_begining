...
from .serializers import ToDoSerializers, UpdateToDoSerializer
...
@api_view(['GET'])
def retreve(request: Request, pk):
    try:
        queryset = ToDo.objects.get(pk=pk)
    except ToDo.DoesNotExist:
        return Response({'detail': f'Object {pk} does not exist'}, status=status.HTTP_404_NOT_FOUND)
    serializer = ToDoSerializers(queryset)
    return Response(serializer.data, status=status.HTTP_200_OK)


@api_view(['GET'])
def get_todo(request: Request, pk):
    todo = get_object_or_404(ToDo, pk=pk)
    serializer = ToDoSerializers(instance=todo)
    return Response(serializer.data, status=status.HTTP_200_OK)

@swagger_auto_schema(method='POST', request_body=ToDoSerializers)
@api_view(['POST'])
